...
/**
*	@file		map_instance_pvp_match.cpp
*	@author		zhjl
...
const DWORD FIRSTBIWUTITLE_M = 23;
const DWORD FIRSTBIWUTITLE_W = 24;
const DWORD BIWU_ACTIVITY_ID = 19;

map_instance_pvp_biwu::map_instance_pvp_biwu(void) : map_instance(), m_pInstance(NULL),
m_dwStartTick(INVALID_VALUE), m_dwEndTick(INVALID_VALUE),
m_bNoEnter(TRUE)
{
}

map_instance_pvp_biwu::~map_instance_pvp_biwu(void)
{
	//destroy();
}
...
// 初始化(不必须要创建者)
//------------------------------------------------------------------------------------------------------
BOOL map_instance_pvp_biwu::init(const tag_map_info* pInfo, DWORD dwInstanceID, Role* pCreator, DWORD dwMisc)
{
	ASSERT( VALID_POINT(pInfo) );
	ASSERT( EMT_PVP_BIWU == pInfo->e_type );
...
// 设置副本ID
//------------------------------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::set_instance_id(DWORD dwInstanceID, INT nActID)
{
...
// 更新
//---------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::update()
{
	/*if( map_role.empty() )
		set_delete();*/
...
// 销毁
//---------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::destroy()
{
...
//---------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::add_role(Role* pRole,  Map* pSrcMap)
{
...
// 玩家离开地图，只可能在主线程里面调用
//---------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::role_leave_map(DWORD dwID, BOOL b_leave_online/* = FALSE*/)
{
...
// 是否能进入副本
//---------------------------------------------------------------------------------
INT	map_instance_pvp_biwu::can_enter(Role *pRole, DWORD dwParam)
{
	// 先检测通用判断
	...
	// 比武活动处于准备阶段，准备阶段可以进入，活动开始后不允许进入
...
// 是否可以删除
//---------------------------------------------------------------------------------
BOOL map_instance_pvp_biwu::can_destroy()
{
...
// 副本销毁
//-----------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::on_destroy()
{
...
// 副本结束
//---------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::on_delete()
{
	// 移除所有在地图内的玩家
...
// 初始化刷怪点怪物
//---------------------------------------------------------------------------------
BOOL map_instance_pvp_biwu::init_all_spawn_point_creature(DWORD dwGuildID/* = INVALID_VALUE*/)
{
...
// 时限相关的更新
//---------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::update_time_issue()
{
	// 如果已经处于待删除状态，就不更新了
	if( is_delete() ) return;
	...
	activity_fix* pActivity = activity_mgr::GetInstance()->get_activity(BIWU_ACTIVITY_ID);	
	if (pActivity != NULL)
	{
		if ( pActivity->is_start() && pActivity->get_minute_update_count() >= 5 )
		{
			if (1==get_role_num())
			{
				package_map<DWORD, Role*>::map_iter it = map_role.begin();
				Role* pRole = NULL;
				if (map_role.find_next(it, pRole) && VALID_POINT(pRole))
				{
					BYTE bySex = pRole->GetSex();
					if (0==bySex)
						pRole->SetTitle(FIRSTBIWUTITLE_W);
					else if (1==bySex)
						pRole->SetTitle(FIRSTBIWUTITLE_M);

					//发送公告
					HearSayHelper::SendMessage(EHST_ACTIVITY_BIWU_WIN,pRole->GetID(),bySex);
				}	
...
// 计算时限副本所剩时间
//-----------------------------------------------------------------------------------
DWORD map_instance_pvp_biwu::cal_time_limit()
{
...
VOID map_instance_pvp_biwu::on_leave_instance(DWORD dw_role_id)
{
...
// 活动结束时
//-----------------------------------------------------------------------------------
VOID map_instance_pvp_biwu::on_act_end()
{
	package_map<DWORD, Role*>::map_iter it = map_role.begin();
	Role* pRole = NULL;
	while (map_role.find_next(it, pRole))
